...
class NewsItem extends Component {
    ...
    submitContent = () => {
        const {  title, displayno, fileString, delFiles, time, id, description } = this.state
    ...
    render() {
        const { description, title, displayno, fileList, time } = this.state
        const { closeItem, detailData } = this.props
        return (
            <Card title={detailData ? '编辑新闻动态' : '新增新闻动态'} bordered={false} style={{ width: '100%' }}>
            <Row>
                <Col span={24}>
